<?php require '_partials/header.php' ?>

<!----seccion 1---->
<div class="section_before_menu">
  <div class="texto_container">
    <div class="texto_section">
      <div class="container_small_0">
        <div class="container_small_1">
          <h1>
            Conoce más<br />
            Sobre nuestro catálogo
          </h1>
        </div>
        <div class="container_small_2">
          <button class="container_icon_2">
            <p class="icon_verde">HOLA, ¿CÓMO PODEMOS <span><br>AYUDARTE?</span> </p>
            <img class="icon_call" src="./assets/img/icon_call.png" alt="telefono verde icono" />
          </button>
        </div>
      </div>
    </div>
  </div>
</div>
</div>
</div>
<div class="banner-container_background_n">
  <div class="banner_2_n"></div>
</div>
<!-----slider-catalogo-ampliacion------>
<!--------------slider------------------>
<div class="container_catalogo_ampliacion">
  <div class="container_swiper_catalogo">
    <!-- Slider main container -->
    <div class="container_catalogo_3">
      <div class="container_slider swiper-catalogo swiper swiper-custom ">
        <!-- Additional required wrapper -->
        <div class="swiper-wrapper">
          <!-- Slides -->
          <!--1-->
          <div class="swiper-slide">
            <div class="swiper_slider_1">
              <div class="swiper_content_slider_catalogo">
                <!---->
                <div class="content_catalogo_01">
                  <img class="img_col_1_catalogo" src="./assets/img/slider_img.png" alt="adobe-vista-previa" />
                  <div class="content_catalogo_col_1">
                    <h3 class="catalogo_tittle">Adobe 10 Liviano</h3>
                    <div class="ref_share_catalogo">
                      <p>referencia: JG111155 |</p>
                      <p>SHARE</p>
                      <img class="share_icon_catalogo" src="./assets/img/icon_share.png" alt="" />
                    </div>
                    <p>
                      The Hepworth Terracotta is manufactured from clay, a finely grained
                      natural rock or soil material that combines one or more clay
                      material with possible traces of quarts and organic material. The
                      firebrick is baked and are treated to withstand heat. Refractory
                      operation such as furnace lining and constructing fireplaces. These
                      firebrick are resistant to fading.
                    </p>
                    <br />
                    <div class="line_black">
                      <hr />
                    </div>
                    <br /><br />
                    <p>
                      Medida nominal (cm): 10x20x40 Peso (kg) : 5 Rendimiento
                      (unidades/m3): 12.5
                    </p>
                    <br />
                    <div class="line_black">
                      <hr />
                    </div>
                    <br /><br />
                    <div class="section_2">
                      <div>
                        <button class="container_icon_3">
                          <p class="text_style_black">
                            descubre más <br />proyectos aquí
                          </p>
                          <img class="icon_mas" src="./assets/img/icon_.png" alt="telefono icono verde" />
                        </button>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <!--2-->
          <div class="swiper-slide">
            <div class="swiper_slider_1">
              <div class="swiper_content_slider_catalogo">
                <!---->
                <div class="content_catalogo_01">
                  <img class="img_col_1_catalogo" src="./assets/img/slider_img.png" alt="adobe-vista-previa" />
                  <div class="content_catalogo_col_1">
                    <h3 class="catalogo_tittle">Adobe 10 Liviano</h3>
                    <div class="ref_share_catalogo">
                      <p>referencia: JG111155 |</p>
                      <p>SHARE</p>
                      <img class="share_icon_catalogo" src="./assets/img/icon_share.png" alt="" />
                    </div>
                    <p>
                      The Hepworth Terracotta is manufactured from clay, a finely grained
                      natural rock or soil material that combines one or more clay
                      material with possible traces of quarts and organic material. The
                      firebrick is baked and are treated to withstand heat. Refractory
                      operation such as furnace lining and constructing fireplaces. These
                      firebrick are resistant to fading.
                    </p>
                    <br />
                    <div class="line_black">
                      <hr />
                    </div>
                    <br /><br />
                    <p>
                      Medida nominal (cm): 10x20x40 Peso (kg) : 5 Rendimiento
                      (unidades/m3): 12.5
                    </p>
                    <br />
                    <div class="line_black">
                      <hr />
                    </div>
                    <br /><br />
                    <div class="section_2">
                      <div>
                        <button class="container_icon_3">
                          <p class="text_style_black">
                            descubre más <br />proyectos aquí
                          </p>
                          <img class="icon_mas" src="./assets/img/icon_.png" alt="telefono icono verde" />
                        </button>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <!--3-->
          <div class="swiper-slide">
            <div class="swiper_slider_1">
              <div class="swiper_content_slider_catalogo">
                <!---->
                <div class="content_catalogo_01">
                  <img class="img_col_1_catalogo" src="./assets/img/slider_img.png" alt="adobe-vista-previa" />
                  <div class="content_catalogo_col_1">
                    <h3 class="catalogo_tittle">Adobe 10 Liviano</h3>
                    <div class="ref_share_catalogo">
                      <p>referencia: JG111155 |</p>
                      <p>SHARE</p>
                      <img class="share_icon_catalogo" src="./assets/img/icon_share.png" alt="" />
                    </div>
                    <p>
                      The Hepworth Terracotta is manufactured from clay, a finely grained
                      natural rock or soil material that combines one or more clay
                      material with possible traces of quarts and organic material. The
                      firebrick is baked and are treated to withstand heat. Refractory
                      operation such as furnace lining and constructing fireplaces. These
                      firebrick are resistant to fading.
                    </p>
                    <br />
                    <div class="line_black">
                      <hr />
                    </div>
                    <br /><br />
                    <p>
                      Medida nominal (cm): 10x20x40 Peso (kg) : 5 Rendimiento
                      (unidades/m3): 12.5
                    </p>
                    <br />
                    <div class="line_black">
                      <hr />
                    </div>
                    <br /><br />
                    <div class="section_2">
                      <div>
                        <button class="container_icon_3">
                          <p class="text_style_black">
                            descubre más <br />proyectos aquí
                          </p>
                          <img class="icon_mas" src="./assets/img/icon_.png" alt="telefono icono verde" />
                        </button>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <!-- If we need pagination -->
    <div>
      <div class="swiper-pagination catalogo-pagination"></div>
    </div>
  </div>
</div>

<!-----------------slider_productos----------------------->
<div class="container_slider">
  <div class="container_slider_1">
    <div class="catalogo_text">
      <h2>productos relacionados</h2>
    </div>
    <div class="container_slider_2">
      <div class="slider_1">
        <div class="content_slider">
          <div class="content_1">
            <h3 class="h3_style_products">Adobe 10 Liviano</h3>
            <img class="img_content" src="./assets/img/icon_mas.png" alt="imagen de producto">
          </div>
          <p class="font_style_products">
            Medida nominal (cm): 10x20x40
            Peso (kg) : 5 Rendimiento
            (unidades/m3): 12.5
          </p>
        </div>
        <img src="./assets/img/slider_img.png" alt="imagen de producto" class="hover_img" id="img1">
      </div>

      <div class="slider_1">
        <div class="content_slider">
          <div class="content_1">
            <h3 class="h3_style_products">Tolete 12 Rayado</h3>
            <img class="img_content" src="./assets/img/icon_mas.png" alt="imagen de producto">
          </div>
          <p class="font_style_products">
            Medida nominal (cm): 10x20x40
            Peso (kg) : 5 Rendimiento
            (unidades/m3): 12.5
          </p>
        </div>
        <img src="./assets/img/slider_img.png" alt="" class="hover_img" id="img2">
      </div>

      <div class="slider_1">
        <div class="content_slider">
          <div class="content_1">
            <h3 class="h3_style_products">Tolete 15 Rayado</h3>
            <img class="img_content" src="./assets/img/icon_mas.png" alt="imagen de producto">
          </div>
          <p class="font_style_products">
            Medida nominal (cm): 10x20x40
            Peso (kg) : 5 Rendimiento
            (unidades/m3): 12.5
          </p>
        </div>
        <img src="./assets/img/slider_img.png" alt="imagen de producto" class="hover_img" id="img3">
      </div>
    </div>
  </div>
</div>
<!-----fin-slider-catalogo-ampliacion--------->


<?php require '_partials/vamos_a_trabajar.php' ?>
<?php require '_partials/footer.php' ?>