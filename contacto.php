<?php require '_partials/header.php' ?>
<!----seccion 1---->
<div class="section_before_menu">
  <div class="texto_container">
    <div class="texto_section">
      <div class="container_small_0">
        <div class="container_small_1">
          <h1>
            Ponte en contacto<br />con nuestros asesores
          </h1>
        </div>
        <div class="container_small_2">
          <button class="container_icon_2">
            <p class="icon_verde">HOLA, ¿CÓMO PODEMOS <span><br>AYUDARTE?</span> </p>
            <img class="icon_call" src="./assets/img/icon_call.png" alt="telefono verde icono" />
          </button>
        </div>
      </div>
    </div>
  </div>
</div>
</div>
</div>
<div class="banner-container_background_n">
  <div class="banner_2_n">
  </div>
</div>
<!----container con 2 col----->

<div class="container_2_col">
  <div class="container_col_section">
    <div class="container_col_section_">
      <div class="container_col_01">
        <div class="container_col_text_1">
          <h1 class="h1_col_02">¿Está interesado en<br>comprar, trabajar<br>con nosotros?</h1> <br>
          <p>CALLE 18# 94 - 65 BELÉN ALTVISTA MEDELLÍN COLOMBIA</p>
          <br><br>
          <p>(604) 475 7140
            <br>(+57) 301 000 00 00
          </p>
          <br><br> <a href="INFO@SANTA-CECILIA.COM">INFO@SANTA-CECILIA.COM</a>
          <a href="VENTAS@SANTA-CECILIA.COM">VENTAS@SANTA-CECILIA.COM</a>
        </div>
        <div class="container_col_02"> <img class="img_col_02" src="./assets/img/nosotros/bodega.png" alt=""> </div>
      </div>
    </div>
  </div>
</div>
<!---contactos---->
<div class="contacto_container">
  <div class="contacto_container_01">
    <div class="contacto_container_02">
      <h1 class="h1_col_blue">Escríbenos o llámanos</h1>
      <div class="contacto_container_03">
        <div class="medio_contenedor">
          <div class="inputs_formulario">
            <div class="input_container">
              <input type="text" placeholder="01. Nombre completo">
              <img src="./assets/img/icon-pencil.png" alt="">
            </div>
            <div class="input_container">
              <input type="email" placeholder="02. correo electrónico">
              <img src="./assets/img/icon-pencil.png" alt="">
            </div>
            <div class="input_container">
              <input type="tel" placeholder="03. teléfono / movil">
              <img src="./assets/img/icon-pencil.png" alt="">
            </div>
          </div>
          <div class="text_area">
            <textarea placeholder="04. Déjanos un mensaje"></textarea>
          </div>
          <div class="terms_conditions">
            <input type="checkbox" id="datos">
            <label for="datos">He leído y acepto las condiciones de tratamiento de datos // clic aquí</label>
          </div>
          <div class="button_container">
            <button type="button">enviar mensaje</button>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>
</div>
<?php require '_partials/footer.php' ?>